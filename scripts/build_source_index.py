...
主要内容:
- scan_source_reports: 扫描 references/source-reports 下的 Markdown 文件
...
def scan_source_reports() -> set[str]:
    return {to_repo_path(path) for path in SOURCE_DIR.glob("*.md")}
...
def main() -> int:
    ...
    if not SOURCE_DIR.exists():
        ...
        if entry_count != 23:
            print("error: expected indexed_reports_count=23")
            return 1
